...
def train_epoch(sess, trainable_model, num_iter,
    ...
    if unsupervised_gen_x is None:
        print (" ")
    else:
        words_in_unsupervised = [words[x] if words else x for x in unsupervised_gen_x] if unsupervised_gen_x is not None else None
        make_sentence = ''.join(map(str, words_in_unsupervised))
        extends_words.append(make_sentence)