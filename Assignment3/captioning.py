...
class Captioning():
    ...
    def train(self, img_features, captions):
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True)), graph=self.graph) as sess:
            ...
            print("Total EPOCH: %d" % self.epoch)
            best_loss = 1000
            for e in range(self.epoch):
                ...
                if e % 5 == 0:
                    print("LOCAL EPOCH : %d, LOSS: %.4f" %(e,loss))
                
            # SAVE the MODEL
            saver = tf.train.Saver(max_to_keep=3)
            saver.save(sess, "./models_captioning/captioning_model", global_step=self.global_step_tensor.eval())
